...
//? блок куда добавляется карточки из функции render
...
function drawPaginationButtons() {
  fetch(`${API}?q=${searchVal}`).then((res) => res.json()).then((data) => {
    pageTotalCount = Math.ceil(data.length / 3)// общее количество продуктов делим на количество продуктов которые отображаются на одной странице
...
searchInp.addEventListener("input", () => {
  searchVal = searchInp.value // записывает значение в переменную searchVal
  currentPage = 1;
  render();
})